...
- **`unlock` verifies the registry** (the plan 1c final-review carry-over): for every account carrying a `Derivation`, re-derive `lock_args` through its `LockModule` and compare. Any mismatch is `CoreError::RegistryMismatch { account_id }`. Cost is one PBKDF2 plus n derivations, single-digit milliseconds. `accounts.json` is plaintext by design, so this is the cheapest integrity guarantee that does not require authenticating the file.
